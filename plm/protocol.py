...
class ALDB(object):
    ...
    def setattr(self, key, attr, value):
        ...
        if key in self._devices:
            ...
            if value != oldvalue:
                self.log.debug('Device %s.%s changed: %s->%s"',
...
class PLM(asyncio.Protocol):
    ...
    def __init__(self, loop=None, connection_lost_callback=None):
        ...
        self._all_link_updated_callbacks = []
        self._poll_completed_callbacks = []
        ...
        if self._buffer.find(sentmessage) == 0:
            ...
            if acknak == 0x06:
                ...
                if code == 0x6a:
                    ...
                    for callback in self._all_link_updated_callbacks:
                        self.log.info('all link db updated callback %s', callback)
    ...
    def _parse_insteon_standard(self, msg):
        ...
        self.log.debug('INSTEON standard %r->%r: cmd1:%02x cmd2:%02x flags:%02x',
    ...
    def _parse_status_response(self, msg):
        ...
        self.log.debug('INSTEON device status %r is at level %s',
    ...
    def _insteon_on(self, msg, device):
        self.log.debug('INSTEON on event: %r, %r', msg, device)
        ...
        if device.get('model') == '2477D':
            ...
            if self.devices.setattr(msg.address, attribute, value):
                request_again = False
                # lets get the real & final value if it's a light...
                if 'light' in device.get('capabilities') or 'dimmable' in device.get('capabilities'):
                    if device['onlevel'] is None:
                        request_again = True
                    if device['onlevel'] != value:
                        request_again = True

                    if request_again:
                        self._loop.call_later(.1, self.status_request, msg.address)
                    else:
                        self._do_update_callback(msg)
    ...
    def _insteon_off(self, msg, device):
        self.log.debug('INSTEON off event: %r,,,, %r', msg, device)
    ...
    def add_poll_completed_callback(self, callback):
        """Register a callback for when a the all-link db has been updated."""
        self._poll_completed_callbacks.append(callback)
        self.log.info('Added poll completed callback to %s', callback)

    def add_all_link_updated_callback(self, callback):
        """Register a callback for when a the all-link db has been updated."""
        self._all_link_updated_callbacks.append(callback)
        self.log.info('Added ALL-Link updated callback to %s', callback)
    ...
    def status_request(self, addr, cmd2='00'):
        ...
        # print("doing status message for: %s" % addr)
        ...
        self.log.debug('Requesting status for %r', address)

        callback = self._parse_status_response
        if device.get('model') == '2450':
            if cmd2 == '00':
                self._loop.call_later(1, self.status_request, address)
            else:
    ...
    def turn_off(self, address, fast=None, ramprate=None):
        ...
        if fast == 'fast' or fast is True:
            ...
            if ramprate is not None:
                self.turn_on(address, brightness=0, ramprate=ramprate)
            else:
                self.send_insteon_standard(address, '13', '00', {})

    def turn_on(self, address, brightness=255, ramprate=None, fast=None):
    ...
    def brighten(self, address):
        """Send command to device to turn off."""
        print("doing brighten")
        self.send_insteon_standard(address, '15', '00', {})

    def dim(self, address):
        """Send command to device to turn off."""
        self.send_insteon_standard(address, '16', '00', {})
    ...
    async def _poll_devices(self, poll_delay = None):
        """Walk through ALDB and populate device information for each device."""
        ...
        for address in self.devices:
            ...
            await asyncio.sleep(poll_delay / 2)
        self.poll_in_progress = False
        self._call_poll_completed_callbacks()

    def _call_poll_completed_callbacks(self):
        for callback in self._poll_completed_callbacks:
            self.log.info('poll completed callback %s', callback)
            self._loop.call_soon(callback)
